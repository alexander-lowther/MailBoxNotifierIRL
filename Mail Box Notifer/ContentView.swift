...
struct TaskSessionSummary: Identifiable {
   let id: String
   let type: String
   ...
   var subtitle: String {
       let startStr = startedAt.formatted(date: .abbreviated, time: .shortened)
       return "\(startStr)"
   }
}
...
struct HomeView: View {
   let userUID: String
   @State private var recentSessions: [TaskSessionSummary] = []
               ...
               if !recentSessions.isEmpty {
                   VStack(alignment: .leading, spacing: 8) {
                       ...
                       ForEach(recentSessions) { session in
                           NavigationLink {
                               SessionDetailView(userUID: userUID, sessionId: session.id)
                           } label: {
                               HStack {
                                   VStack(alignment: .leading, spacing: 2) {
                                       Text(session.useCaseName.isEmpty ? session.type : session.useCaseName)
   ...
   func subscribeSessions() {
       ...
       db.collection("users").document(uid)
           ...
           .addSnapshotListener { snap, _ in
               ...
               recentSessions = docs.compactMap { doc in
                   let data = doc.data()
                   let type = data["type"] as? String ?? "Session"
                   let useCaseName = data["useCaseName"] as? String ?? ""
                   let started = (data["startedAt"] as? Timestamp)?.dateValue() ?? Date()
                   let ended = (data["endedAt"] as? Timestamp)?.dateValue()
...
struct FunctionsView: View {
   ...
   private var items: [FunctionItem] {
       [
           .init(
               ...
               status: .available,
               info: "Uses screen auto-brightness change (no camera) to detect openings or light changes. Sends push notifications to all signed-in devices "
           ),
      /*     .init(
               title: "Air Pressure BARROMETER",
   ...
   private var header: some View {
       VStack(alignment: .leading, spacing: 6) {
           Text("Create a Task")
               .font(.title2.bold())
           Text("Choose a Task to turn your device into a customizable sensor or notifier")
               .font(.subheadline)
               .foregroundStyle(.secondary)
       }
   }
...
struct FunctionCard: View {
   ...
   private var displaySubtitle: String  {
       if item.title == "Mailbox Notifier" {
           return "Sudden changes in brightness "
       }
       return item.subtitle
   }
    private var displaySubtitle2: String  {
        if item.title == "Mailbox Notifier" {
            return "(MailBox, Cabinet, Car)"
        }
        if item.title == "Vibration Sensor" {
            return "(Dryer & Washing Machine)"
        }
        if item.title == "Sound Sensor" {
            return "Dog Bark, House"
        }        else {
            return ""
        }
       ...
       VStack(alignment: .leading, spacing: 10) {
           ...
           if #available(iOS 17.0, *) {
               Text(displaySubtitle)
                   .font(.caption)
                   .foregroundStyle(.secondary) + Text(displaySubtitle2).foregroundStyle(Color(.systemBlue))
           } else {
               // Fallback on earlier versions
               Text(displaySubtitle)
                   .font(.caption)
                   .foregroundColor(.secondary) + Text(displaySubtitle2).foregroundColor(Color(.systemBlue))           }
       }
       .padding(14)
...
struct LevelSensorConfig {
   ...
   let sendNotifications: Bool   // NEW
}
...
struct PowerLossConfig {
   ...
   var sendNotifications: Bool?   // NEW
}
...
struct LevelListeningView: View {
   ...
   @State private var localSamples: [SessionSamplePoint] = []
   private let cooldown: TimeInterval = 10
   ...
   var body: some View {
       VStack(spacing: 16) {
           ...
           Text("This sensor uses device motion to estimate tilt. When the phone tilts past your target angle (e.g. 45°), we'll update this device and, if enabled, send your custom notification.")
               .font(.footnote)
               .foregroundStyle(.secondary)
               .multilineTextAlignment(.center)
               .padding(.top, 4)
   ...
   private func fireLevelEvent(angle: Double) {
       ...
       // Always record the event in Firestore for history/analytics.
       db.collection("users").document(uid)
           .setData(["lastLevelEventAt": FieldValue.serverTimestamp()],
                    merge: true)
       
       // Only send push notification if user wants notifications.
       guard config.sendNotifications,
             let url = URL(string: "https://us-central1-notifymailbox-d9657.cloudfunctions.net/sendMailNotification")
       else { return }
   ...
   private func sendLifecycleNotification(starting: Bool) {
       // Respect sendNotifications for lifecycle pushes.
       guard config.sendNotifications,
       ...
       let title = starting
           ...
           : "Stopped listening: \(config.useCaseName)"
       let body = starting
           ? "This device is now monitoring tilt."
           : "This device stopped monitoring tilt."
   ...
   private func startSession() {
       ...
       let sessionsRef = db.collection("users").document(uid)
           .collection("sessions")
       let doc = sessionsRef.document()
       ...
       doc.setData(payload, merge: false)
   }

   private func recordSample(angle: Double) {
       ...
       db.collection("users").document(uid)
           ...
           .updateData([
               "samples": FieldValue.arrayUnion([point])
           ])
   }
       ...
       VStack(alignment: .leading, spacing: 16) {
           ...
           NavigationLink(isActive: $pushToListening) {
               ...
               let cfg = MagnetSensorConfig(
                   ...
                   thresholdDelta: clamped, sendNotifications: true
               )
...
struct LevelSensorSetupView: View {
   ...
   var body: some View {
       VStack(alignment: .leading, spacing: 16) {
           ...
           VStack(alignment: .leading, spacing: 10) {
               Bullet(
                  "Keep the app open",
                  detail: "Ensure that the app remains running in the foreground"
               )
               
               Bullet(
                "Secure the Phone",
                detail: "Place your device on a solid surface"
               )
           }
           .padding(12)
           .background(.ultraThinMaterial)
   ...
   private var allRequiredChecks: Bool {
       disableAutoLock && securePlacement
   }
   ...
   private func loadConfig() {
       ...
       db.collection("users").document(uid)
           ...
           .getDocument { snap, _ in
               ...
               if let send = data["sendNotifications"] as? Bool {
...
struct SecurityCameraSetupView: View {
   ...
   @State private var config = SecurityCameraConfig(
       ...
       uploadIntervalSeconds: 3, sendNotifications: true
   )
   @State private var pushToListening = false
...
struct MailboxNotifierSetupView: View {
   ...
   @State private var config = MailboxNotifierConfig(
       ...
       notificationTitle: "You've recieved mail!",
   ...
   var body: some View {
       VStack(alignment: .leading, spacing: 16) {
           ...
           VStack(alignment: .leading, spacing: 8) {
               ...
               Bullet(
                   "Place phone face-up",
                   detail: "Stable position, in an area with little to no movement. "
               )
               Bullet(
                   "App must be running with the screen unlocked",
                   detail: "Settings → Display & Brightness → Auto-Lock -> Never"
               )
           ...
           VStack(alignment: .leading, spacing: 8) {
               ...
               TextField("Use-case name (e.g. Front mailbox)", text: $config.useCaseName).textFieldStyle(.roundedBorder).background(Color(.systemGray6))
               Text("Customize Notification").font(.headline).bold()

               Text("Subject").font(.caption).fontWeight(.semibold).foregroundColor(.secondary)
               TextField("Notification title", text: $config.notificationTitle)
                   .textFieldStyle(.roundedBorder).background(Color(.systemGray6))
               Text("Body").font(.caption).fontWeight(.semibold).foregroundColor(.secondary)
               
               TextField("Notification body", text: $config.notificationBody)
                   .textFieldStyle(.roundedBorder).background(Color(.systemGray6))
               
            //   Toggle("Send notifications to other devices", isOn: $config.sendNotifications).font(.subheadline)
      //     }
       }
           if !hasStartedTimer && !pushToListening {
               Button {
                   ...
                   Label("Activate", systemImage: "timer")
                       .frame(maxWidth: .infinity)
               }
               .buttonStyle(.borderedProminent)
           }
           ...
           if hasStartedTimer && !pushToListening {
               VStack(spacing: 8) {
                   Text("Place the device in the desired area.")
                       .font(.subheadline)
                       .foregroundStyle(.secondary)
                   Text("\(countdown)")
...
struct MailboxListeningView: View {
   ...
   var body: some View {
       VStack(spacing: 16) {
           HStack {
               ...
               VStack(alignment: .leading) {
                   Text(hasTriggered ? "Mail Detected" : "Listening changes")
                       .font(.title3.bold())
                   Text(status)
                       .font(.caption)
                       .foregroundStyle(.secondary)
   ...
   private func fireMailEvent() {
       ...
       // Still mark mail detected in Firestore so UI/history can use it.
       db.collection("users").document(uid)
           .setData(["mailDetected": true], merge: true)
       
       // Respect "sendNotifications"
       guard config.sendNotifications,
             let url = URL(string: "https://us-central1-notifymailbox-d9657.cloudfunctions.net/sendMailNotification")
       else { return }
   ...
   private func sendLifecycleNotification(starting: Bool) {
       // Only send lifecycle notifications when user wants notifications.
       guard config.sendNotifications,
             let uid = Auth.auth().currentUser?.uid,
             let url = URL(string: "https://us-central1-notifymailbox-d9657.cloudfunctions.net/sendMailNotification")
       else { return }
...
struct VibrationListeningView: View {
   ...
   var body: some View {
       VStack(spacing: 16) {
           ...
           Text("Device is acting as a vibration sensor. Spikes in vibration will update this device and, if enabled, send your custom notification to other signed-in devices.")
               .font(.footnote)
               .foregroundStyle(.secondary)
               .multilineTextAlignment(.center)
               .padding(.top, 4)
   ...
   private func fireVibrationEvent() {
       ...
       // Only send push if user wants notifications.
       guard config.sendNotifications,
             let url = URL(string: "https://us-central1-notifymailbox-d9657.cloudfunctions.net/sendMailNotification")
       else { return }
   ...
   private func sendLifecycleNotification(starting: Bool) {
       // Respect the sendNotifications flag for lifecycle messages.
       guard config.sendNotifications,
             let uid = Auth.auth().currentUser?.uid,
             let url = URL(string: "https://us-central1-notifymailbox-d9657.cloudfunctions.net/sendMailNotification")
       else { return }
...
struct SoundListeningView: View {
   ...
   var body: some View {
       VStack(spacing: 16) {
           ...
           Text("Device is acting as a sound sensor. When the sound level crosses your threshold, this device will react and, if enabled, send your custom notification.")
               .font(.footnote)
               .foregroundStyle(.secondary)
               .multilineTextAlignment(.center)
               .padding(.top, 4)
   ...
   private func fireSoundEvent() {
       ...
       // Always record event for UI/history.
       db.collection("users").document(uid)
       ...
       VStack(alignment: .leading, spacing: 16) {
           ...
           NavigationLink(isActive: $pushToListening) {
               let config = PresenceSensorConfig(
                   ...
                   notificationBody: notificationBody.isEmpty ? "Movement or handling was detected near this device." : notificationBody, sendNotifications: true
               )
...
struct SessionSamplePoint: Identifiable {
   let id = UUID()
   let time: Date
   let value: Double
}
...
struct SessionDetailView: View {
   ...
   var body: some View {
       ScrollView {
           VStack(alignment: .leading, spacing: 16) {
               Text(useCaseName.isEmpty ? type : useCaseName)
                   .font(.title2.bold())
               ...
               if !samples.isEmpty {
                   ...
                   Text("No recorded samples for this session.")
                       .font(.footnote)
                       .foregroundStyle(.secondary)
               }
   ...
   private func loadSession() {
       db.collection("users").document(userUID)
           ...
           .getDocument { snap, _ in
               ...
               if let rawSamples = data["samples"] as? [[String: Any]] {
                   samples = rawSamples.compactMap { dict in
                       guard let ts = dict["t"] as? Timestamp,
                             let v = dict["v"] as? Double else { return nil }
                       return SessionSamplePoint(time: ts.dateValue(), value: v)
                   }
               }
           }
   }
...
// MARK: - Height Change Sensor (Step 2 Only)

import CoreMotion

struct HeightChangeSetupView: View {
    @State private var thresholdFeet: String = "5"
    @State private var isListening: Bool = false
    @State private var startAltitude: Double?
    @State private var statusText: String = "Not listening"
    ...
    var body: some View {
        VStack(spacing: 16) {
            ...
            Text("Notify me when my phone raises or drops a set number of feet.")
                .font(.subheadline)
                .multilineTextAlignment(.center)

            TextField("Feet threshold", text: $thresholdFeet)
                .keyboardType(.decimalPad)
    ...
    private func startListening() {
        guard CMAltimeter.isRelativeAltitudeAvailable() else {
            statusText = "Altitude sensor not available"
            return
        }

        isListening = true
        statusText = "Listening for height change..."

        altimeter.startRelativeAltitudeUpdates(to: .main) { data, error in
            guard let data = data else { return }

            if startAltitude == nil {
                startAltitude = data.relativeAltitude.doubleValue
                return
            }

            let deltaMeters = data.relativeAltitude.doubleValue - (startAltitude ?? 0)
            let deltaFeet = abs(deltaMeters * 3.28084)

            if let threshold = Double(thresholdFeet), deltaFeet >= threshold {
                statusText = "Height change detected: " + String(deltaFeet) + "Feet"
                stopListening()
                // Notification trigger should hook into existing notification system
            }
        }
    }
...
FunctionsView(userUID: userUID)
.tabItem { Label("Tasks", systemImage: "square.grid.2x2.fill") }
...
struct HomeView: View {
   ...
   private let db = Firestore.firestore()
    struct SessionDetailView: View {
       ...
       @State private var endedAt: Date?
       @State private var samples: [SessionSamplePoint] = []
        @State private var threshold: Double? = nil
    ...
    var body: some View {
       ...
       .onAppear {
           ...
           subscribeActiveTask()
           subscribeSessions()    // NEW
            trimSessionsHistory() // NEW hard cap
       }
   }
   ...
   private var activeTaskCard: some View {
       VStack(alignment: .leading, spacing: 8) {
           ...
           if activeTasksSummary.isEmpty {
               Text("Start a Task in the Tasks tab (Light Change, Vibration, Sound, Power Loss, Security Camera) to begin listening.")
                   .font(.caption)
                   .foregroundStyle(.secondary)
           } else if activeCameraDeviceID != nil {
               Text("Tap to view the live camera feed from this device.")
...
/// Hard cap: keep only the 5 most recent sessions in Firestore.
/// This protects against unbounded growth post-launch.
func trimSessionsHistory() {
     let db = Firestore.firestore()
    guard let uid = Auth.auth().currentUser?.uid else { return }
    db.collection("users").document(uid)
        .collection("sessions")
        ...
        .getDocuments { snap, _ in
            guard let docs = snap?.documents, docs.count > 5 else { return }
            let toDelete = docs.dropFirst(5)
            let batch = db.batch()
   ...
   private var items: [FunctionItem] {
       [
           .init(
               title: "Mailbox Notifier",
               subtitle: "Detect mail / light change",
               systemImage: "markunread_mailbox",
               status: .available,
               info: "Uses screen auto-brightness change (no camera) to detect openings or light changes. Sends push notifications to all signed-in devices "
           ),
      /*     .init(
           ...
           .init(
               ...
               systemImage: "waveform.path.ecg",
               status: .available,
               info: "Use the accelerometer to detect vibration from appliances, tools, vehicles, footsteps, or other movement."
           ),
           .init(
               title: "Sound Sensor",
               subtitle: "Noise / knock detection",
   ...
   var body: some View {
       VStack(alignment: .leading, spacing: 10) {
           ...
           if #available(iOS 17.0, *) {
               Text(displaySubtitle)
                   .font(.caption)
                   .foregroundStyle(.secondary) + Text(displaySubtitle2).font(.caption).foregroundStyle(Color(.systemBlue))
           } else {
               // Fallback on earlier versions
               Text(displaySubtitle)
                   .font(.caption)
               .foregroundColor(.secondary) + Text(displaySubtitle2).font(.caption).foregroundColor(Color(.systemBlue))           }
       }
       .padding(14)
       .frame(maxWidth: .infinity, minHeight: 110, alignment: .topLeading)
       .background(.thinMaterial)
...
@State private var devices: [Device] = []
private let db = Firestore.firestore()
    func trimNotificationHistory() {
        db.collection("users").document(userUID).collection("notifications")
            .order(by: "createdAt", descending: true)
            .limit(to: 200)
            .getDocuments { snap, _ in
                guard let docs = snap?.documents, docs.count > 20 else { return }
                let toDelete = docs.dropFirst(20)
                let batch = db.batch()
                toDelete.forEach { batch.deleteDocument($0.reference) }
                batch.commit(completion: nil)
            }
...
struct NotificationsView: View {
    ...
    var body: some View {
        NavigationStack {
            ...
            .onAppear { subscribe(); trimNotificationHistory() }
        }
    }
    ...
    /// Hard cap: keep only the 20 most recent notifications in Firestore.
    /// This protects against unbounded growth post-launch.
...
struct LevelListeningView: View {
    ...
    private func fireLevelEvent(angle: Double) {
        ...
        // Enforce "5 notifications per task" using notifCount on the session doc.
        guard let sid = sessionId else { return }
        let sessionRef = db.collection("users").document(uid)
            .collection("sessions").document(sid)
        
        sendEventNotificationCapped(title: "Notifications Stopped", body: "You have reached the notification limit of 5 for this task")
        
        
    }
    
    // MARK: - Notification cap (max 5 per session/task)
    
    /// Returns codes:
    /// 0 = already at/over limit, do nothing
    /// 1 = incremented, send normal notification
    /// 2 = hit the 5th, send "limit reached" notification
    private func incrementNotifCountAndGetCode(sessionRef: DocumentReference, completion: @escaping (Int) -> Void) {
        db.runTransaction({ (txn, errorPointer) -> Any? in
            let snap: DocumentSnapshot
            do {
                snap = try txn.getDocument(sessionRef)
            } catch let err as NSError {
                errorPointer?.pointee = err
                return nil
            }
            
            let current = snap.data()?["notifCount"] as? Int ?? 0
            ...
            if current == 4 {
                txn.updateData(["notifCount": 5], forDocument: sessionRef)
                return 2 as NSNumber
            } else {
                txn.updateData(["notifCount": current + 1], forDocument: sessionRef)
                return 1 as NSNumber
            }
        }) { result, _ in
            let code = (result as? NSNumber)?.intValue ?? 0
            completion(code)
        }
    }
    
    private func sendEventNotificationCapped(title: String, body: String) {
        guard let uid = Auth.auth().currentUser?.uid,
              let sid = sessionId,
              let url = URL(string: "https://us-central1-notifymailbox-d9657.cloudfunctions.net/sendMailNotification")
        else { return }
        
        let ref = db.collection("users").document(uid).collection("sessions").document(sid)
        ...
        db.runTransaction({ (txn, errorPointer) -> Any? in
            ...
            if current == 4 {
                txn.updateData(["notifCount": 5], forDocument: ref)
                return 2 as NSNumber
            } else {
                txn.updateData(["notifCount": current + 1], forDocument: ref)
                return 1 as NSNumber
            }
        })
        { result, _ in
            guard let decision = result as? Int else { return }
            var outgoingTitle = title
            var outgoingBody = body
            if decision == 2 {
                outgoingTitle = "Notication Limit Reached"
                outgoingBody = "You have reached the 5 notifications per task limit. Starting a new task will refresh the limit"
            }
            guard decision == 1 || decision == 2 else { return }
    ...
    private func startSession() {
        // If you support a Record toggle on LevelSensorConfig, use it here.
        // If your config does NOT have `record`, remove `config.record` references and keep unconditional session creation.
        guard sessionId == nil,
              let uid = Auth.auth().currentUser?.uid else { return }

        // If neither record nor notifications are enabled, there’s no reason to create a session doc.
        // (Keeps your database clean post-launch.)
        if (config.sendNotifications == false)  {//&& (config.record == false) {
            return
        }
        ...
        let payload: [String: Any] = [
            ...
            "useCaseName": config.useCaseName,
            "startedAt": FieldValue.serverTimestamp(),
            // Enables the chart threshold line.
            "threshold": config.targetAngle,
            // Required for the 5-notification cap.
            "notifCount": 0,
            // Enables saving sessions without notifications.
    ...
    private func recordSample(angle: Double) {
        // Respect Record toggle.
     //   guard config.record else { return }
...
struct LevelSensorSetupView: View {
    ...
    // NOTE: these were being used to gate the Start button, but were never set anywhere.
    // We keep them (since your logic references them) and expose them as simple confirmations.
    @State private var disableAutoLock = false
    @State private var securePlacement = false
    ...
    @State private var notificationBody: String = ""
    @State private var angleString: String = "45"
    @State private var thresholdString: String = "0.10"   // extra threshold value
    @State private var sendNotifications: Bool = true
    @State private var record: Bool = true
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 10) {
                ...
                // These fix the “button disabled” issue by allowing the user to satisfy allRequiredChecks.
                Toggle("Disable Auto-Lock while listening", isOn: $disableAutoLock)
                    .font(.subheadline)
                
                Toggle("Confirm phone is placed securely", isOn: $securePlacement)
                    .font(.subheadline)
            }
            .padding(12)
            .background(.ultraThinMaterial)
            ...
            if sendNotifications == true {
                VStack(alignment: .leading, spacing: 8) {
                    Text("Customize Task").font(.headline).bold()
                    ...
                    TextField("Use-case name (e.g. Garage door, Shelf, Gate)", text: $useCaseName)
                        .textFieldStyle(.roundedBorder)
                    ...
                    TextField("Notification Subject", text: $notificationTitle)
                        .textFieldStyle(.roundedBorder)
                        .background(Color(.systemGray6))
                    ...
                    Text("Target Tilt Angle")
                        .font(.caption)
                        .fontWeight(.semibold)
                        .foregroundColor(.secondary)
                    
                    TextField("Degrees (e.g. 45)", text: $angleString)
                        .keyboardType(.numberPad)
                        .textFieldStyle(.roundedBorder)
                        .background(Color(.systemGray6))
                    ...
                    TextField("Threshold (e.g. 0.10)", text: $thresholdString)
                        .keyboardType(.decimalPad)
                        .textFieldStyle(.roundedBorder)
                        .background(Color(.systemGray6))
                    
                    Text("Tip: Lower threshold = more sensitive. If you get false triggers, increase it slightly.")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
            ...
            NavigationLink(isActive: $pushToListening) {
                let rawAngle = Double(angleString) ?? 45
                let clampedAngle = max(5, min(85, rawAngle))
                
                let rawThreshold = Double(thresholdString) ?? 0.10
                let clampedThreshold = max(0.01, min(1.0, rawThreshold))
    ...
    private func loadConfig() {
        ...
        db.collection("users").document(uid)
            ...
            .getDocument { snap, _ in
                ...
                if let th = data["threshold"] as? Double {
                    thresholdString = String(format: "%.2f", th)
                }
                if let send = data["sendNotifications"] as? Bool {
                    sendNotifications = send
                }
                if let rec = data["record"] as? Bool {
                    record = rec
                }
            }
    }
    ...
    struct MailboxNotifierSetupView: View {
        ...
        @State private var config = MailboxNotifierConfig(
            useCaseName: "Mailbox Notifier",
            notificationTitle: "You've recieved mail!",
            notificationBody: "Brightness change in your mailbox.",
            sendNotifications: true,
            record: true
        )
        ...
        var body: some View {
            VStack(alignment: .leading, spacing: 16) {
                ...
                VStack(alignment: .leading, spacing: 8) {
                    Bullet(
                        "Enable True Tone",
                        detail: "Settings → Display & Brightness → True Tone On"
                    )
                    Bullet(
                        "Place phone face-up",
                        detail: "Stable position, in an area with little to no movement. "
                ...
                if config.sendNotifications == true {
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        Text("Task Name").font(.caption).fontWeight(.semibold).foregroundColor(.secondary)
                        
                        TextField("Use-case name (e.g. Front mailbox)", text: $config.useCaseName).textFieldStyle(.roundedBorder).background(Color(.systemGray6))
                        
                        
                        Text("Notification Subject").font(.caption).fontWeight(.semibold).foregroundColor(.secondary)
                        
                        TextField("Notification Subject", text: $config.notificationTitle)
                        
                            .textFieldStyle(.roundedBorder).background(Color(.systemGray6))
                        Text("Notification Body").font(.caption).fontWeight(.semibold).foregroundColor(.secondary)
                        
                        TextField("Notification Body", text: $config.notificationBody)                         .textFieldStyle(.roundedBorder).background(Color(.systemGray6))
                ...
                Toggle("Send notifications to other devices", isOn: $config.sendNotifications)
    ...
    struct MailboxListeningView: View {
        ...
        var body: some View {
            VStack(spacing: 16) {
                HStack {
                    Image(systemName: hasTriggered ? "envelope.badge.fill" : "ear.badge.waveform")
                        ...
                        .foregroundStyle(hasTriggered ? .green : .blue)
                    VStack(alignment: .leading) {
                        Text(hasTriggered ? "Detected Change" : "Light Changes")
                            .font(.title3.bold())
                        Text(status)
                            .font(.caption)
                            .foregroundStyle(.secondary)
            ...
            .onAppear {
                baseline = max(UIScreen.main.brightness, 0.001)
                current = UIScreen.main.brightness
                status = "Task has been activated"
                UIApplication.shared.isIdleTimerDisabled = true
        ...
        private func sampleAndEvaluate() {
            current = UIScreen.main.brightness
            if config.record {
                let p = SessionSamplePoint(time: Date(), value: Double(current))
                samples.append(p)
        ...
        private func fireMailEvent() {
            ...
            // Respect "sendNotifications"
            guard config.sendNotifications else { return }
            sendEventNotificationCapped(title: config.notificationTitle, body: config.notificationBody)
        }
        ...
        private func startSession() {
            guard let uid = Auth.auth().currentUser?.uid else { return }
            let ref = db.collection("users").document(uid).collection("sessions").document()
            sessionId = ref.documentID
            
            let thresholdValue = Double(baseline + absoluteDelta)
            let payload: [String: Any] = [
                "type": "Light Change",
                "useCaseName": config.useCaseName,
                "startedAt": FieldValue.serverTimestamp(),
...
{ result, _ in
    ...
    struct VibrationSensorSetupView: View {
        ...
        @State private var pushToListening = false
        @State private var config = VibrationSensorConfig(
            useCaseName: "Vibration Changes",
            notificationTitle: "Your Laundry is Done",
            notificationBody: "Your clothes are dry",
            sendNotifications: true,
            record: true
        )
        private let db = Firestore.firestore()
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                if sendNotifications == true {
                    ...
                    VStack(alignment: .leading, spacing: 8) {
                        ...
                        TextField("Use-case name (e.g. Dryer)", text: $config.useCaseName).textFieldStyle(.roundedBorder).background(Color(.systemGray6))
    ...
    struct VibrationListeningView: View {
        ...
        @StateObject private var detector = VibrationDetector()
        private let absoluteDelta: CGFloat = 0.0025
        ...
        private func startSession() {
            ...
            let thresholdValue = Double(0.003)
            let payload: [String: Any] = [
                "type": "Light Change",
                "useCaseName": config.useCaseName,
                "startedAt": FieldValue.serverTimestamp(),
...
{ result, _ in
        ...
        private func startSampling() {
            ...
            sampleTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { _ in
                let p = SessionSamplePoint(time: Date(), value: detector.variance)
                samples.append(p)
                // Keep memory bounded; sessions are capped to 5 anyway, but long runs can still grow.
                if samples.count > 300 {
                    samples.removeFirst(samples.count - 300)
                }
            }
...
struct SoundSensorSetupView: View {
        ...
        VStack(alignment: .leading, spacing: 16) {
            ...
            if sendNotifications == true {
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    TextField("Use-case name (e.g. Knocks, Alarm, Room noise)", text: $useCaseName)
                        .textFieldStyle(.roundedBorder)
                        .background(Color(.systemGray6))
                    ...
                    Text("Detects spikes automatically")
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
            }
    ...
    struct SoundListeningView: View {
        ...
        private func startSampling() {
            ...
            sampleTimer?.invalidate()
            sampleTimer = Timer.scheduledTimer(withTimeInterval: 1.0, repeats: true) { _ in
                let p = SessionSamplePoint(time: Date(), value: 0.003)
                samples.append(p)
                // Keep memory bounded; sessions are capped to 5 anyway, but long runs can still grow.
                if samples.count > 300 {
                    samples.removeFirst(samples.count - 300)
...
{ result, _ in
        ...
        private func fireSoundEvent() {
            ...
            // Only send push if user wants notifications.
            guard config.sendNotifications == true else { return }
            sendEventNotificationCapped(title: config.notificationTitle, body: config.notificationBody)
        }
